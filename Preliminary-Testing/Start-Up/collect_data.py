...
if __name__ == '__main__':
    ...
    if not start_bool:
        ...
        time.sleep(1) # Allow time to adjust before data collection

        #print("Starting collection") # status update
        filename = "start_up_data_60.csv"
        accel_cal(total_time=60, FILENAME=filename) 
        ...
        print("Means: ", np.mean(x_accels), " ", np.mean(y_accels), " ", np.mean(z_accels))
        print("Std D: ", np.std(x_accels),  " ", np.std(y_accels),  " ", np.std(z_accels))
